...
if __name__ == '__main__':
    ...
    check(opt)
    path = os.path.join('pretrained/',  opt.task, opt.stage, 'latest_net_G.pth')
    path = os.path.join('pretrained/backup',  opt.task, opt.stage, 'latest_net_G.pth')
#     path = os.path.join('pretrained/backup/horse2zebra/full/latest_net_G.pth')
    download(path)
    if opt.stage != 'compressed':
        path = os.path.join('pretrained\backup', opt.model, opt.task, opt.stage, 'latest_net_D.pth')
#         path = 'pretrained/backup/horse2zebra/full/latest_net_D.pth'
        download(path)
